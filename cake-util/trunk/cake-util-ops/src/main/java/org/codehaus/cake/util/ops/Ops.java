...
/**
 * Interfaces and utilities declaring per-element operations used within parallel methods on aggregates. This class
 * provides type names for all operation signatures accepting zero, one or two arguments, and returning zero or one
 * results, for parameterized types, as well as specializations to <tt>int</tt>, <tt>long</tt>, and
 * <tt>double</tt>. In keeping with normal Java evaluation rules that promote, for example <tt>short</tt> to
 * <tt>int</tt>, operation names for these smaller types are absent.
 * 
 * <p>
 * <b>Preliminary release note: Some of the declarations in this class are likely to be moved elsewhere in the JDK
 * libraries upon actual release, and most likely will not all nested in the same class.</b>
 * 
 * <p>
 * The naming conventions are as follows:
 * <ul>
 * 
 * <li> The name of the single method declared in each interface is simply <tt>op</tt> (short for "operate").
 * 
 * <li> An <tt>Op</tt> (short for "operation") maps a single argument to a result. Example: negating a value.
 * 
 * <li> The names for scalar ops accepting and returning the same type are prefaced by their type name.
 * 
 ...
 * Example: testing if a number is even.
 * 
 * <li>A <tt>BinaryPredicate</tt> accepts two values and returns a boolean indicator that the arguments obeys some
 * relation. Example: testing if two numbers are relatively prime.
 * 
 * <li>Scalar versions of {@link Comparator} have the same properties as the Object version -- returning negative,
 * zero, or positive if the first argument is less, equal, or greater than the second.
 * 
 * </ul>
 * 
 ...
 * 
 * <p>
 * In addition to stated signatures, implementations of these interfaces must work safely in parallel. In general, this
 * means methods should operate only on their arguments, and should not rely on ThreadLocals, unsafely published
 * globals, or other unsafe constructions. Additionally, they should not block waiting for synchronization.
 * 
 * <p>
...
public final class Ops {
    ...
    public static interface BinaryFloatOp {
