 ...
 * computed earlier, where the original data are expensive (usually in terms of access time) to
 * fetch or compute relative to reading from the cache. Once the data are stored in the cache,
 ...
 * cache. Furthermore, the cache will <tt>not</tt> check whether or not an entry has expired when
 * calling methods on any of the collection views. As a result the cache might return values that
 * have expired.
 * <p>
 * All general-purpose <tt>Cache</tt> implementation classes should provide two "standard"
 * constructors: a void (no arguments) constructor, which creates an empty cache with default
 * settings, and a constructor with a single argument of type {@link CacheConfiguration}. There is
 * no way to enforce this recommendation (as interfaces cannot contain constructors) but all of the
 * general-purpose cache implementations available in Cake Cache comply. Unlike its super class
 * {@link java.util.Map}, a constructor taking a single map is not required.
 * <p>
 * Generally, Cache implementations do not define element-based versions of the <tt>equals</tt>
...
public interface Cache<K, V> extends ConcurrentMap<K, V>, Container {
     ...
     * formally, returns <tt>true</tt> if and only if this cache contains a mapping for a key
     * <tt>k</tt> such that <tt>(key==null ? k==null : key.equals(k))</tt>. (There can be at
     * most one such mapping.)
     * <p>
     * This method does not check the expiration status of an element and will return if a expired
     * element is present in the cache for the specified key.
     * 
     * @param key
     *            key whose presence in this cache is to be tested
     * @return <tt>true</tt> if this cache contains a mapping for the specified key, otherwise
     *         <tt>false</Tt>
    ...
    /**
     * Returns <tt>true</tt> if this cache maps one or more keys to the specified value. More
     * formally, returns <tt>true</tt> if and only if this cache contains at least one mapping to
     * a value <tt>v</tt> such that <tt>(value==null ? v==null : value.equals(v))</tt>. This
     * operation will probably require time linear in the cache size for most implementations of the
     * <tt>Cache</tt> interface.
     * <p>
     * This method does not check the expiration status of an element and will return if a expired
     * element is present in the cache for the specified value.
     * 
     * @param value
    ...
    /**
     * Returns a {@link Set} view of the mappings contained in this cache. The set is backed by the
     * cache, so changes to the cache are reflected in the set, and vice-versa. If the cache is
     * modified while an iteration over the set is in progress (except through the iterator's own
     * <tt>remove</tt> operation, or through the <tt>setValue</tt> operation on a map entry
     * returned by the iterator) the results of the iteration are undefined. The set supports
     ...
     * <p>
     * Unlike {@link Cache#get(Object)} no methods on the view checks if an element has expired. For
     * example, iterating though values the view might return an expired element.
     * <p>
    ...
    /**
     * Attempts to retrieve all of the mappings for the specified collection of keys. The effect of
     * this call is equivalent to that of calling {@link #get(Object)} on this cache once for each
     * key in the specified collection. However, in some cases it can be much faster to load several
     * cache items at once, for example, if the cache must fetch the values from a remote host.
     * <p>
     * If a value is not contained in the cache and the value cannot be loaded by any of the
     * configured cache backends. The returned map will contain a mapping from the key to
     * <tt>null</tt>.
     * <p>
     * The behavior of this operation is unspecified if the specified collection is modified while
     * the operation is in progress.
     * 
     * @param keys
     *            a collection of keys whose associated values are to be returned.
     * @return a map with mappings from each key to the corresponding value, or to <tt>null</tt>
     *         if no mapping for this key exists.
     * @throws ClassCastException
     *             if any of the keys in the specified collection are of an inappropriate type for
     *             this cache (optional).
     * @throws NullPointerException
     ...
     * @return the cache entry to which this cache maps the specified key, or <tt>null</tt> if the
     ...
     * <p>
     * If this cache has not been started a call to this method will automatically start it. If this
     * cache has been shutdown this method returns <tt>true</tt>.
     * 
    ...
    /**
     * Returns a {@link Set} view of the keys contained in this cache. The set is backed by the
     * cache, so changes to the cache are reflected in the set, and vice-versa. If the cache is
     * modified while an iteration over the set is in progress (except through the iterator's own
     * <tt>remove</tt> operation), the results of the iteration are undefined. The set supports
     * element removal, which removes the corresponding mapping from the cache, via the
     * <tt>Iterator.remove</tt>, <tt>Set.remove</tt>, <tt>removeAll</tt>,
     * <tt>retainAll</tt>, and <tt>clear</tt> operations. It does not support the <tt>add</tt>
     * or <tt>addAll</tt> operations.
     * <p>
     * Unlike {@link Cache#get(Object)} no methods on the view checks if an element has expired. For
     * example, iterating though key set the view might return a key for an expired element.
     * <p>
     * If the cache has been shutdown calls to <tt>Iterator.remove</tt>,
     * <tt>Collection.remove</tt>, <tt>removeAll</tt>, <tt>retainAll</tt> and <tt>clear</tt>
     * operation will throw an IllegalStateException.
    ...
    /**
     * This method works analogues to the {@link #get(Object)} method with the following
     * modifications.
     * <p>
     * It will not try to fetch missing items, it will only return a value if it actually exists in
     * the cache. Furthermore, it will not effect the statistics gathered by the cache. Finally,
     * even if the item is expired it will still be returned.
     * <p>
     * All implementations of this method should take care to assure that a call to peek does not
     * have any unforeseen side effects. For example, it should not modify some state in addition to
     * returning a value or not returning a value.
     * 
     * @param key
     *            key whose associated value is to be returned.
    ...
    /**
     * This method works analogues to the {@link #peek(Object)} method. However, it will return a
     * cache entry instead of just the value.
     * 
     * @param key
    ...
    /**
     * Associates the specified value with the specified key in this cache (optional operation). If
     * the cache previously contained a mapping for this key, the old value is replaced by the
     * specified value. (A cache <tt>c</tt> is said to contain a mapping for a key <tt>k</tt> if
     * and only if {@link org.codehaus.cake.cache.Cache#containsKey(Object) c.containsKey(k)} would
     * return <tt>true</tt>.))
     * <p>
    ...
    /**
     * Copies all of the mappings from the specified map to this cache (optional operation). The
     * effect of this call is equivalent to that of calling {@link #put(Object,Object) put(k, v)} on
     * this cache once for each mapping from key <tt>k</tt> to value <tt>v</tt> in the specified
    ...
    /**
     * Removes the mapping for a key from this cache if it is present (optional operation). More
     * formally, if this cache contains a mapping from key <tt>k</tt> to value <tt>v</tt> such
     * that <code>(key==null ?  k==null : key.equals(k))</code>, that mapping is removed. (The
     * cache can contain at most one such mapping.)
     * <p>
     * Returns the value to which this cache previously associated the key, or <tt>null</tt> if
    ...
    /**
     * Attempts to remove all of the mappings for the specified collection of keys. The effect of
     * this call is equivalent to that of calling
     * {@link org.codehaus.cake.cache.Cache#removeByIndex(Object)} on the cache once for each key in
     * the specified collection. However, in some cases it can be much faster to remove several
     * cache items at once, for example, if some of the values must also be removed on a remote
     * host.
     * 
     * @param keys
     ...
     *             if the <tt>put</tt> operation is not supported by this cache
     * @throws ClassCastException
     *             if the class of a specified key or value prevents it from being stored in this
     *             cache
     * @throws NullPointerException
     ...
     *             if the cache has been shutdown
     * @throws IllegalArgumentException
     *             if some property of a specified key or value prevents it from being stored in
     *             this cache
     */
     ...
     * <p>
     * If this cache has not been started calling this method will automatically start it. If this
     * cache has been shutdown this method returns <tt>0</tt>.
     * 
    ...
    /**
     * Returns a {@link Collection} view of the values contained in this cache. The collection is
     * backed by the cache, so changes to the cache are reflected in the collection, and vice-versa.
     * If the cache is modified while an iteration over the collection is in progress (except
     * through the iterator's own <tt>remove</tt> operation), the results of the iteration are
     * undefined. The collection supports element removal, which removes the corresponding mapping
     * from the cache, via the <tt>Iterator.remove</tt>, <tt>Collection.remove</tt>,
     * <tt>removeAll</tt>, <tt>retainAll</tt> and <tt>clear</tt> operations. It does not
