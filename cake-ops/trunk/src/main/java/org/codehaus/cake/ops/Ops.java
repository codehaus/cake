...
/**
 * Interfaces and utilities declaring per-element operations used within parallel methods on
 * aggregates. This class provides type names for all operation signatures accepting zero, one or
 * two arguments, and returning zero or one results, for parameterized types, as well as
 * specializations to <tt>int</tt>, <tt>long</tt>, and <tt>double</tt>. In keeping with
 * normal Java evaluation rules that promote, for example <tt>short</tt> to <tt>int</tt>,
 * operation names for these smaller types are absent.
 * 
 * <p>
 * <b>Preliminary release note: Some of the declarations in this class are likely to be moved
 * elsewhere in the JDK libraries upon actual release, and most likely will not all nested in the
 * same class.</b>
 * 
 * <p>
 ...
 * <ul>
 * 
 * <li> The name of the single method declared in each interface is simply <tt>op</tt> (short for
 * "operate").
 * 
 * <li> An <tt>Op</tt> (short for "operation") maps a single argument to a result. Example:
 * negating a value.
 * 
 * <li> The names for scalar ops accepting and returning the same type are prefaced by their type
 * name.
 * 
 * <li> A <tt>BinaryOp</tt> maps two arguments to a result. Example: dividing two numbers
 * 
 * <li>A <tt>Reducer</tt> is an <em>associative</em> binary op accepting and returning values
 * of the same type; where op(a, op(b, c)) should have the same result as op(op(a, b), c). Example:
 * adding two numbers.
 * 
 ...
 * 
 * <li> A <tt>Generator</tt> takes no arguments and returns a result. Examples: random number
 * generators, builders
 * 
 * <li> A <tt>Procedure</tt> accepts an argument but doesn't return a result. Example: printing a
 * value. An <tt>Action</tt> is a Procedure that takes no arguments.
 * 
 * <li>A <tt>Predicate</tt> accepts a value and returns a boolean indicator that the argument
 * obeys some property. Example: testing if a number is even.
 * 
 * <li>A <tt>BinaryPredicate</tt> accepts two values and returns a boolean indicator that the
 * arguments obeys some relation. Example: testing if two numbers are relatively prime.
 * 
 * <li>Scalar versions of {@link Comparator} have the same properties as the Object version --
 * returning negative, zero, or positive if the first argument is less, equal, or greater than the
 * second.
 * 
 * </ul>
 * 
 * <table border=1 cellpadding=0 cellspacing=0 > <caption>result = op(a) or result = op(a,b)</caption>
 * <tr>
 * <td >
 * <td >
 * <td >
 ...
 * 
 * <p>
 * In addition to stated signatures, implementations of these interfaces must work safely in
 * parallel. In general, this means methods should operate only on their arguments, and should not
 * rely on ThreadLocals, unsafely published globals, or other unsafe constructions. Additionally,
...
public final class Ops {
    ...
    public static interface FloatOp                     { float  op(float a);}
    public static interface BinaryFloatOp               { float  op(float a, float b);}
